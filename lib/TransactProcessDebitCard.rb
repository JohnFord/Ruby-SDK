...
#Sale
#CardNum - The debit card number used for the transaction.
#ExpDate - The expiration date of the debit card used for the transaction in MMYY format.
...
#Amount - The dollar amount of the transaction in DDDDDDDDDD.CC format. This amount includes any cash back or surcharge amounts and any tax or tip amounts specified in ExtData.
#InvNum - The invoice number used by the merchant to identify the transaction.
#PNRef - Not used for sale transactions.
...
#CashBackAmt - The amount in DDDD.CC format that a cardholder requests for cash back
#RegisterNum - Not used for sale transactions.
#ExtData - An XML string containing additional data for the transaction. See the following section for more information.
  def ProcessDebitCard(url,username,password,transtype,cardnum,expdate,magdata,nameoncard,amount,invnum,pnref,pin,surechargeamt,cashbackamt,registernum,extdata)
